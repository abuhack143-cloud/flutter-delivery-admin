...
import 'package:admin/data/datasource/remote/order/cancel.dart';
import 'package:admin/data/model/deliveryModel.dart';
import 'package:admin/data/model/location.dart';
...
class MapLocationControllerImp extends GetxController {
  ...
  CancelData orderData = CancelData(Get.find());
  ...
  Future<void> getdata() async {
    data.clear();
    var response = await orderData.getData(deliveryid.toString());
    ...
    if (StatusRequest.success == statusRequest) {
      if (response['status'] == "success") {
        ...
        if (responseData is List) {
          data.addAll(responseData.map((e) => LocationModel.fromJson(e)));
        } else if (responseData is Map) {
          Map<String, dynamic> mapData =
              Map<String, dynamic>.from(responseData);
          data.add(LocationModel.fromJson(mapData));
        }
      } else {
      ...
      data.map(
        (item) async {
          double? lat = double.tryParse(item.locationLat.toString());
          double? lng = double.tryParse(item.locationLng.toString());
          if (lat != null && lng != null) {
            PaintImage paintImage = PaintImage();
            BitmapDescriptor customIcon = await paintImage.getMarkerIconFromUrl(
              AppImageAsset.deleviryImage + item.deliveryImage.toString(),
              "images/marker_red.png",
            );
            final markerId = MarkerId(item.loactionId.toString());
            final marker = Marker(
              markerId: markerId,
