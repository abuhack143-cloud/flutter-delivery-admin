...
class MapDeliveryControllerImp extends GetxController {
  ...
  Pending2Data orderData = Pending2Data(Get.find());
  ...
  void connectToWebSocket() {
    ...
    socketService!.onLocationReceived = (location) async {
      ...
      final exists = locations.any((existingLocation) =>
          ...
          existingLocation['isdelivery'] == location['isdelivery']);
      print(location);
      if (!exists) {
  ...
  Future<void> approveOrders(String deliveryid) async {
    var response = await orderData.approve2Order(orderid.toString(),
  ...
  Future<void> getdelivery(String id) async {
    ...
    var response = await orderData.deliverymap(id);
